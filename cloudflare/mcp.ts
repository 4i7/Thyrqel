...
export async function handleMcp(request: Request, broker: DurableObjectStub<import('./index.js').Broker>) {
  ...
  server.registerTool('device_status', {
    description: 'Check device connectivity and its current process epoch. An epoch change means prior outcomes cannot be recovered; never resubmit old actions automatically.',
    inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false },
  }, async () => reply(await broker.deviceStatus()));
  server.registerTool('terminal_submit', {
    description: 'Submit one terminal operation. First obtain device_status. Supply a fresh UUID operationId and its epoch; retain both. Submission does not mean execution or command completion. Retrieve terminal_result for the same ID. Do not repeat uncertain actions with a new ID. Input is exact PTY text; append carriage return to submit a line, U+0003 for Ctrl+C. No command-name filter. Do not put passwords into tool arguments.',
