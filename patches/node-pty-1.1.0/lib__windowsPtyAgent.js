...
var WindowsPtyAgent = /** @class */ (function () {
    function WindowsPtyAgent(file, args, env, cwd, cols, rows, debug, _useConpty, _useConptyDll, conptyInheritCursor) {
        ...
        if (this._useConpty) {
            ...
            if (conptyNative.thyrqelLifecyclePatch !== 1) throw new Error('Unpatched ConPTY native module; run npm ci');
        }
        else {
            if (!winptyNative) {
        ...
        this._outSocket = new net_1.Socket({ allowHalfOpen: this._useConpty });
        this._closing = false;
        this._outputEnded = false;
        this._cleanedUp = false;
        this._outSocket.on("end", function () {
            _this._outputEnded = true;
            _this._cleanUpProcess();
        });
    ...
    WindowsPtyAgent.prototype.kill = function () {
        ...
        if (this._useConpty) {
            ...
            this._closing = true;
            // Close the owned pseudoconsole, not a racy snapshot of reusable PIDs.
            // Keep draining output on the worker until every attached client disconnects.
    ...
    WindowsPtyAgent.prototype._$onProcessExit = function (exitCode) {
        ...
        // The patched native callback retains the baton until this call returns.
        this.kill();
        this._cleanUpProcess();
    };
    WindowsPtyAgent.prototype._cleanUpProcess = function () {
        if (this._cleanedUp || !this._outputEnded || this._exitCode === undefined) return;
