...
void SetupExitCallback(Napi::Env env, Napi::Function cb, pty_baton* baton) {
  ...
  *th = std::thread([tsfn = std::move(tsfn), baton] {
    auto callback = [](Napi::Env env, Napi::Function cb, ExitEvent *exit_event) {
      // Only the JS thread owns the registry. Retain HPCON until JS closes it.
      pty_baton* handle = get_pty_baton(exit_event->pty_id);
      CloseHandle(handle->hShell);
      handle->hShell = nullptr;
      cb.Call({Napi::Number::New(env, exit_event->exit_code)});
      if (!remove_pty_baton(exit_event->pty_id)) {
        Napi::Error::Fatal("SetupExitCallback", "Missing PTY baton");
      }
      delete exit_event;
    };
...
Napi::Object init(Napi::Env env, Napi::Object exports) {
  exports.Set("thyrqelLifecyclePatch", Napi::Number::New(env, 1));
